 ...
 * <p>Description: This is a subset of the normal RungeKutta derivation with a increment change.</p>
...
public class RungeKuttaInc extends MathCalc
{
   public static String IDENTIFIER = "RungeKutta4+Inc.";

   public RungeKuttaInc(Vector strings)
   {
      super(strings);
   }
   ...
   public Vector calcValue(double x0, double xf, Vector startvector, double h, double eps)
   {
      ...
      while ( x < xf )
      {
        ...
        // now we have to calculate the q value
        double q=0;
        double z=0;
        for(int i=0; i < numFuncs; i++)
        {
          ...
          // q := Abs((k3 - k2)/(k2 - k1))
          q = q + Math.abs((k3_tmp-k2_tmp)/(k2_tmp-k1_tmp));

          double x11 = calcDEQ(parser, x + h/2.0, y);
          double x12 = calcDEQ(parser, x + h, y);

          z = z + (x11 - x12)/x11;
          //System.out.println("i:"+i+" x11:"+x11+" x12:"+x12+" z:"+z);
        }

        q = q / numFuncs;
        z = z / numFuncs;
        //System.out.println("h:"+h+" q:"+q+" z:"+z);
